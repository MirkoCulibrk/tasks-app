...
import { List, Task } from "../redux/types";
import { useDispatch } from "react-redux";
import { unsetTaskToEdit, editTask } from "../redux/actions/listActions";
...
const EditTaskModal: FC<EditTaskModalProps> = ({
  ...
  const submitHandler = (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (taskName.trim() === "") {
      return alert("taskName is requred");
    }
    if (taskName.trim() === task.name && taskState === task.completed) {
      return alert("Task name and state are the same as before");
    }
    dispatch(editTask(task.id, taskName, taskState, list));
